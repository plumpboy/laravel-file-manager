...
function performLfmRequest(url, parameter, type = 'json', method = 'GET') {
  ...
  return $.ajax({
    ...
    beforeSend: function(request) {
      request.setRequestHeader("Authorization_Bearer", 'Bearer ' + window.accessToken);
    },
    dataType: type,
    url: lfm_route + '/' + url,
    data: data,
...
function loadFolders() {
  performLfmRequest('folders', {}, 'html', 'GET')
    .done(function (data) {
      $(`#tree>ul[node="${data.node}"]`).html(data.dir);
      loadItems();
    });
}
...
function loadItems() {
  loading(true);
  performLfmRequest('items', {show_list: show_list, sort_type: sort_type}, 'html', 'GET')
    .done(function (data) {
      selected = [];
...
function createFolder(folder_name) {
  performLfmRequest('makeDir', {name: folder_name})
    .done(refreshItems);
}
